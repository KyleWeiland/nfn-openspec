...
- `requirements.in` lists the direct dependencies with compatible version ranges.
  **Edit this file** to add, remove or re-range a dependency.
...
updates `requirements.in` and `requirements.txt`, plus separate PRs for security
fixes. The **Dependency check** workflow installs the lock and imports every pipeline
module on each PR that touches `scripts/`. A green check means the upgrade loads, not
